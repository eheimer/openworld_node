  ...
  try {
    ...
    sails.lift(configOverrides, (err)=>{
      ...
      sails.helpers.http.sendHttpRequest.with({
        ...
        if (err) {
          //return done(new Error('Test runner could not fetch CSRF token.\nDetails:\n'+err.stack));
        }

        // console.log(serverResponse);

        var csrfToken = '5ofSzSuk-82tc94zdjZJX9n9y10VUBtecXPY';
        try {
          csrfToken = JSON.parse(serverResponse.body)._csrf;
        } catch (unusedErr) {
          //return done(new Error('Test runner could not parse CSRF token from the Sails server.\nDetails:\n'+err.stack));
        }
        ...
        var sailsSidCookie = 's%3AUgb8X3P10fc28PSKetgqu9LxOwd875SV.EHurMYDWRYBFK0uhOwIqZZYnDnmHLDOG0YM4LGgkSGE';
        try {
          ...
          //return done(new Error('Test runner could not parse the `set-cookie` response header from the Sails server.\nDetails:\n'+err.stack));
        }
        ...
        try {
          ...
          Cloud.setup({
            ...
            methods: require('./CLOUD_SDK_METHODS')
          });
